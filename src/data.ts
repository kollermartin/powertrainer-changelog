...
export const changelogEntries: ChangelogEntry[] = [
    {
        ...
        date: "2026-09-04",
        title: {
            en: "Your notifications now come by email too",
            cs: "Vaše oznámení teď přicházejí i e-mailem",
            de: "Deine Benachrichtigungen kommen jetzt auch per E-Mail",
            fr: "Vos notifications arrivent maintenant aussi par e-mail",
            ru: "Ваши уведомления теперь приходят и по почте",
            uk: "Ваші повідомлення тепер надходять і на пошту",
            pl: "Twoje powiadomienia przychodzą teraz także e-mailem",
        },
        changes: [
            {
                type: "added",
                text: {
                    en: "Everything in your notification list now also arrives as an email — a training assigned or completed, feedback on one, a coaching request and its answer, a new review or a changed rating, an invitation answered, an athlete leaving a group. Each email is written in your language and carries a button that opens exactly the page it is about. Your notification list in the app is unchanged, and so are group invitations: those already send their own email with the link to accept.",
                    cs: "Všechno, co máte v seznamu oznámení, teď přijde i e-mailem — zadaný nebo dokončený trénink, zpětná vazba k němu, žádost o vedení a odpověď na ni, nová recenze nebo změněné hodnocení, odpověď na pozvánku, odchod sportovce ze skupiny. Každý e-mail je ve vašem jazyce a má tlačítko, které otevře přímo tu stránku, o které e-mail je. Seznam oznámení v aplikaci se nemění a nemění se ani pozvánky do skupiny: ty už svůj vlastní e-mail s odkazem na přijetí posílají.",
                    de: "Alles, was in deiner Benachrichtigungsliste steht, kommt jetzt auch als E-Mail — ein zugewiesenes oder abgeschlossenes Training, Feedback dazu, eine Coaching-Anfrage und ihre Antwort, eine neue Bewertung oder eine geänderte Note, eine beantwortete Einladung, ein Sportler, der eine Gruppe verlässt. Jede E-Mail ist in deiner Sprache geschrieben und hat einen Button, der genau die Seite öffnet, um die es geht. An der Benachrichtigungsliste in der App ändert sich nichts, und an Gruppeneinladungen auch nicht: die schicken längst ihre eigene E-Mail mit dem Link zum Annehmen.",
                    fr: "Tout ce qui figure dans votre liste de notifications arrive désormais aussi par e-mail — un entraînement assigné ou terminé, un retour à son sujet, une demande d'accompagnement et sa réponse, un nouvel avis ou une note modifiée, une invitation à laquelle on a répondu, un athlète qui quitte un groupe. Chaque e-mail est rédigé dans votre langue et comporte un bouton qui ouvre exactement la page concernée. Votre liste de notifications dans l'application ne change pas, et les invitations de groupe non plus : celles-ci envoient déjà leur propre e-mail avec le lien pour accepter.",
                    ru: "Всё, что есть в вашем списке уведомлений, теперь приходит и письмом — назначенная или завершённая тренировка, обратная связь по ней, запрос на тренерство и ответ на него, новый отзыв или изменённая оценка, ответ на приглашение, уход спортсмена из группы. Каждое письмо написано на вашем языке и содержит кнопку, которая открывает именно ту страницу, о которой идёт речь. Список уведомлений в приложении не меняется, как и приглашения в группу: они и раньше отправляли собственное письмо со ссылкой для принятия.",
                    uk: "Усе, що є у вашому списку повідомлень, тепер надходить і листом — призначене або завершене тренування, зворотний зв'язок щодо нього, запит на тренерство та відповідь на нього, новий відгук або змінена оцінка, відповідь на запрошення, вихід спортсмена з групи. Кожен лист написаний вашою мовою й має кнопку, яка відкриває саме ту сторінку, про яку йдеться. Список повідомлень у застосунку не змінюється, як і запрошення до групи: вони й раніше надсилали власний лист із посиланням для прийняття.",
                    pl: "Wszystko, co masz na liście powiadomień, przychodzi teraz również e-mailem — przypisany albo ukończony trening, informacja zwrotna do niego, prośba o współpracę i odpowiedź na nią, nowa opinia albo zmieniona ocena, odpowiedź na zaproszenie, sportowiec opuszczający grupę. Każdy e-mail jest napisany w Twoim języku i ma przycisk, który otwiera dokładnie tę stronę, o którą chodzi. Lista powiadomień w aplikacji się nie zmienia, podobnie jak zaproszenia do grupy: te już wcześniej wysyłały własny e-mail z linkiem do przyjęcia.",
                },
            },
            {
                type: "added",
                text: {
                    en: "Settings has a new Email notifications tab where you decide which of them reach your inbox: one switch for all notification emails, and under it a switch each for Trainings, Coaching requests, Reviews and Group membership. You only see the kinds that can actually reach you, so as an athlete there are two. Turning them all off keeps your choices below for when you turn them back on, and none of this touches the notification list in the app — everything still shows up there.",
                    cs: "V nastavení je nová záložka E-mailová oznámení, kde si vyberete, která z nich vám mají chodit: jeden přepínač pro všechna e-mailová oznámení a pod ním samostatné přepínače pro Tréninky, Žádosti o vedení, Recenze a Členství ve skupině. Vidíte jen ty druhy, které se k vám vůbec mohou dostat — jako sportovec tedy dva. Když vypnete všechna, vaše volby pod tím zůstanou zachované pro chvíli, kdy je zapnete zpátky. Na seznam oznámení v aplikaci to nemá vliv, tam se dál zobrazuje všechno.",
                    de: "In den Einstellungen gibt es einen neuen Tab E-Mail-Benachrichtigungen, in dem du entscheidest, welche davon in deinem Postfach landen: ein Schalter für alle Benachrichtigungs-E-Mails und darunter je einer für Trainings, Coaching-Anfragen, Bewertungen und Gruppenmitgliedschaft. Du siehst nur die Arten, die dich überhaupt erreichen können — als Sportler also zwei. Schaltest du alle aus, bleiben deine Auswahlen darunter erhalten, bis du sie wieder einschaltest. Die Benachrichtigungsliste in der App bleibt davon unberührt, dort steht weiterhin alles.",
                    fr: "Les paramètres comptent un nouvel onglet Notifications par e-mail, où vous choisissez lesquelles arrivent dans votre boîte : un interrupteur pour tous les e-mails de notification, et en dessous un interrupteur pour Entraînements, Demandes d'accompagnement, Avis et Appartenance au groupe. Vous ne voyez que les catégories qui peuvent réellement vous atteindre — en tant qu'athlète, il y en a donc deux. Si vous les désactivez toutes, vos choix en dessous sont conservés pour le jour où vous les réactiverez. Rien de tout cela ne touche la liste de notifications dans l'application : tout continue d'y apparaître.",
                    ru: "В настройках появилась новая вкладка «Уведомления по эл. почте», где вы выбираете, какие из них будут приходить: один переключатель для всех писем с уведомлениями и под ним отдельные — «Тренировки», «Запросы на тренерство», «Отзывы» и «Участие в группе». Вы видите только те виды, которые вообще могут до вас дойти, — у спортсмена их два. Если выключить все, ваши настройки ниже сохранятся до того момента, когда вы включите письма снова. На список уведомлений в приложении это не влияет — там по-прежнему видно всё.",
                    uk: "У налаштуваннях з'явилася нова вкладка «Повідомлення на пошту», де ви обираєте, які з них надходитимуть: один перемикач для всіх листів із повідомленнями та під ним окремі — «Тренування», «Запити на тренерство», «Відгуки» і «Участь у групі». Ви бачите лише ті види, які взагалі можуть до вас дійти, — у спортсмена їх два. Якщо вимкнути всі, ваші налаштування нижче збережуться до моменту, коли ви ввімкнете листи знову. На список повідомлень у застосунку це не впливає — там і далі видно все.",
                    pl: "W ustawieniach jest nowa zakładka Powiadomienia e-mail, w której decydujesz, które z nich mają do Ciebie trafiać: jeden przełącznik dla wszystkich e-maili z powiadomieniami, a pod nim osobne dla Treningów, Prośb o współpracę, Opinii i Członkostwa w grupie. Widzisz tylko te rodzaje, które w ogóle mogą do Ciebie dotrzeć — jako sportowiec masz więc dwa. Jeśli wyłączysz wszystkie, Twoje wybory poniżej zostaną zachowane na moment, gdy włączysz je z powrotem. Nie ma to wpływu na listę powiadomień w aplikacji — tam nadal widać wszystko.",
                },
            },
            {
                type: "added",
                text: {
                    en: "Every notification email you can switch off carries an unsubscribe link at the bottom, and your mail app's own Unsubscribe button works as well. Either one turns your notification emails off straight away, without signing in. Your choices per kind are kept, so you can sign in later and switch just the ones you want back on.",
                    cs: "Každý e-mail s oznámením, který lze vypnout, má dole odkaz pro odhlášení a funguje i tlačítko Odhlásit odběr ve vašem e-mailovém klientovi. Obojí vypne e-mailová oznámení okamžitě a bez přihlášení. Vaše volby u jednotlivých druhů zůstanou, takže se můžete později přihlásit a zapnout zpátky jen to, co chcete.",
                    de: "Jede abschaltbare Benachrichtigungs-E-Mail hat unten einen Abmelde-Link, und der Abmelden-Button deines E-Mail-Programms funktioniert genauso. Beides schaltet deine Benachrichtigungs-E-Mails sofort aus, ohne Anmeldung. Deine Auswahl je Art bleibt erhalten, du kannst dich also später anmelden und nur das wieder einschalten, was du willst.",
                    fr: "Chaque e-mail de notification que vous pouvez désactiver contient un lien de désinscription en bas, et le bouton Se désabonner de votre logiciel de messagerie fonctionne aussi. L'un comme l'autre coupe vos e-mails de notification immédiatement, sans connexion. Vos choix par catégorie sont conservés : vous pouvez vous connecter plus tard et ne réactiver que ce que vous voulez.",
                    ru: "В каждом письме с уведомлением, которое можно отключить, внизу есть ссылка для отказа, и кнопка «Отписаться» в вашей почтовой программе тоже работает. И то и другое сразу отключает письма с уведомлениями — без входа в аккаунт. Настройки по отдельным видам сохраняются, так что позже вы можете войти и включить обратно только то, что хотите.",
                    uk: "У кожному листі з повідомленням, який можна вимкнути, внизу є посилання для відмови, і кнопка «Відписатися» у вашій поштовій програмі теж працює. І те, і те одразу вимикає листи з повідомленнями — без входу в акаунт. Налаштування за окремими видами зберігаються, тож пізніше ви можете увійти й повернути лише те, що хочете.",
                    pl: "Każdy e-mail z powiadomieniem, który można wyłączyć, ma na dole link do rezygnacji, a przycisk Wypisz się w Twoim programie pocztowym też działa. Jedno i drugie od razu wyłącza e-maile z powiadomieniami, bez logowania. Twoje wybory dla poszczególnych rodzajów zostają, więc możesz później się zalogować i włączyć z powrotem tylko to, co chcesz.",
                },
            },
            {
                type: "improved",
                text: {
                    en: "Emails about billing and account access — a failed payment, a grace period starting, athletes being blocked, Solo ending — are always sent, and deliberately have no switch and no unsubscribe link. They are how your subscription and your sign-in keep working, and a control that does not actually stop an email is worse than none.",
                    cs: "E-maily o platbách a přístupu k účtu — neúspěšná platba, začátek ochranné lhůty, zablokování sportovců, konec Sola — se posílají vždy a záměrně nemají žádný přepínač ani odkaz pro odhlášení. Díky nim vám funguje předplatné a přihlášení, a ovládací prvek, který e-mail ve skutečnosti nezastaví, je horší než žádný.",
                    de: "E-Mails zu Zahlungen und Kontozugang — eine fehlgeschlagene Zahlung, ein beginnender Kulanzzeitraum, gesperrte Sportler, das Ende von Solo — werden immer verschickt und haben bewusst keinen Schalter und keinen Abmelde-Link. Sie sind der Grund, dass dein Abo und deine Anmeldung weiter funktionieren, und ein Schalter, der eine E-Mail nicht wirklich stoppt, ist schlechter als keiner.",
                    fr: "Les e-mails concernant la facturation et l'accès au compte — un paiement échoué, un délai de grâce qui commence, des athlètes bloqués, la fin de Solo — sont toujours envoyés et n'ont volontairement ni interrupteur ni lien de désinscription. C'est grâce à eux que votre abonnement et votre connexion continuent de fonctionner, et un réglage qui n'arrête pas vraiment un e-mail vaut moins que pas de réglage du tout.",
                    ru: "Письма об оплате и доступе к аккаунту — неудачный платёж, начало льготного периода, блокировка спортсменов, окончание Solo — отправляются всегда и намеренно не имеют ни переключателя, ни ссылки для отказа. Именно благодаря им продолжают работать подписка и вход, а переключатель, который на самом деле не останавливает письмо, хуже, чем его отсутствие.",
                    uk: "Листи про оплату та доступ до акаунта — невдалий платіж, початок пільгового періоду, блокування спортсменів, завершення Solo — надсилаються завжди й навмисно не мають ані перемикача, ані посилання для відмови. Саме завдяки їм продовжують працювати підписка та вхід, а перемикач, який насправді не зупиняє лист, гірший за його відсутність.",
                    pl: "E-maile o płatnościach i dostępie do konta — nieudana płatność, początek okresu karencji, zablokowani sportowcy, koniec Solo — wysyłane są zawsze i celowo nie mają ani przełącznika, ani linku do rezygnacji. To dzięki nim dalej działa Twoja subskrypcja i logowanie, a przełącznik, który tak naprawdę nie zatrzymuje e-maila, jest gorszy niż jego brak.",
                },
            },
            {
                type: "improved",
                text: {
                    en: "The language you choose is now saved to your account instead of just this browser, so it follows you to another phone or computer — and it is the language every email is written in. A trainer-group invitation arrives in it too; an invitation sent to somebody who has no account yet is in English.",
                    cs: "Jazyk, který si vyberete, se teď ukládá k vašemu účtu, ne jen do tohoto prohlížeče, takže vás doprovodí i na jiný telefon nebo počítač — a je to jazyk, ve kterém jsou napsané všechny e-maily. Ve stejném jazyce přijde i pozvánka do skupiny trenéra; pozvánka pro někoho, kdo ještě účet nemá, je v angličtině.",
                    de: "Die Sprache, die du wählst, wird jetzt in deinem Konto gespeichert und nicht nur in diesem Browser, sie begleitet dich also auf ein anderes Handy oder einen anderen Computer — und in ihr sind alle E-Mails geschrieben. Auch eine Einladung in eine Trainergruppe kommt in dieser Sprache; eine Einladung an jemanden ohne Konto ist auf Englisch.",
                    fr: "La langue que vous choisissez est maintenant enregistrée sur votre compte et non plus seulement dans ce navigateur : elle vous suit donc sur un autre téléphone ou un autre ordinateur — et c'est la langue dans laquelle tous les e-mails sont rédigés. Une invitation à un groupe de coach arrive aussi dans cette langue ; une invitation envoyée à quelqu'un qui n'a pas encore de compte est en anglais.",
                    ru: "Выбранный вами язык теперь сохраняется в аккаунте, а не только в этом браузере, поэтому он переходит с вами на другой телефон или компьютер — и на нём написаны все письма. На том же языке приходит и приглашение в группу тренера; приглашение тому, у кого пока нет аккаунта, отправляется на английском.",
                    uk: "Обрана вами мова тепер зберігається в акаунті, а не лише в цьому браузері, тому вона переходить із вами на інший телефон чи комп'ютер — і саме нею написані всі листи. Тією ж мовою надходить і запрошення до групи тренера; запрошення тому, хто ще не має акаунта, надсилається англійською.",
                    pl: "Język, który wybierasz, jest teraz zapisywany na Twoim koncie, a nie tylko w tej przeglądarce, więc idzie z Tobą na inny telefon czy komputer — i w tym języku napisane są wszystkie e-maile. W tym samym języku przychodzi też zaproszenie do grupy trenera; zaproszenie do kogoś, kto nie ma jeszcze konta, jest po angielsku.",
                },
            },
        ],
    },
